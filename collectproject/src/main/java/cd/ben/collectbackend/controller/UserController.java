...
public class UserController {
    ...
    @GetMapping("/invetigators")
    public ResponseEntity<?> getAllInvestigators() {
        return new ResponseEntity<List<User>>(userService.allInvestigators(), HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> loginUser(@Valid @RequestBody LoginPayload credentials, BindingResult result) {
        ...
        if (user == null) {
            result.addError(new FieldError("User", "username", "User not found, check your username"));

            for (FieldError error : result.getFieldErrors()) {
                errorMap.put(error.getField(), error.getDefaultMessage());
            }
            return new ResponseEntity<Map<String, String>>(errorMap, HttpStatus.BAD_REQUEST);
//            return new ResponseEntity<String>("User not found. Plz check your username", HttpStatus.BAD_REQUEST);
...
//            result.addError(new FieldError("User", "user", "User not found"));
    ...
    @GetMapping("/{id}/isAssignedToInvestigation/{qID}")
    public ResponseEntity isAssigned(@PathVariable Long id, @PathVariable Long qID) {
        System.out.println("id = " + id);
        User user = userService.findById(id);
        if (user == null) {
            return new ResponseEntity<String>("User not found", HttpStatus.BAD_REQUEST);
//            result.addError(new FieldError("User", "user", "User not found"));
        ...
        List<Questionnaire> allQuestionnaires = (ArrayList<Questionnaire>) userService.getAllQuestionnaires(id);

        long count = allQuestionnaires
                .stream()
                .filter(questionnaire -> questionnaire.getId() == qID)
                .count();
